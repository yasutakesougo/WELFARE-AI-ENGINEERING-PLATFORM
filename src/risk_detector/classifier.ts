...
const GOVERNED_RULES: Rule[] = [
  { boundary: "R1_PRODUCTION", ruleId: "R1-PRODUCTION-WRITE", pattern: /\b(prod(?:uction)?|live)\b[\s\S]{0,80}\b(deploy|write|update|config|database|db)\b|\b(deploy|write|update|configure)\b[\s\S]{0,80}\b(prod(?:uction)?|live)\b/i },
  { boundary: "R2_AUTHORITY", ruleId: "R2-AUTHORITY-CHANGE", pattern: /\b(permission|role|authority|authorization|authentication|identity|rbac|acl|access)\b[\s\S]{0,80}\b(change|grant|revoke|modify|update|expand|reduce)\b|\b(change|grant|revoke|modify|update|expand|reduce)\b[\s\S]{0,80}\b(permission|role|authority|authorization|authentication|identity|rbac|acl|access)\b/i },
  { boundary: "R3_SENSITIVE", ruleId: "R3-SENSITIVE-HANDLING", pattern: /\b(personal data|pii|sensitive data|credential|secret|api key|token|password|private key)\b[\s\S]{0,100}\b(store|persist|log|send|transfer|handle|expose|write|add)\b|\b(store|persist|log|send|transfer|handle|expose|write|add)\b[\s\S]{0,100}\b(personal data|pii|sensitive data|credential|secret|api key|token|password|private key)\b/i },
  { boundary: "R4_DESTRUCTIVE", ruleId: "R4-DESTRUCTIVE", pattern: /\b(delete|purge|drop|truncate|destroy|overwrite)\b[\s\S]{0,80}\b(data|database|db|table|record|records|resource|storage|bucket|schema)\b|\b(data|database|db|table|record|records|resource|storage|bucket|schema)\b[\s\S]{0,80}\b(delete|purge|drop|truncate|destroy|overwrite)\b|\b(schema|migration)\b[\s\S]{0,80}\b(irreversible|destructive|drop)\b|\b(irreversible|destructive)\b[\s\S]{0,80}\b(schema|migration|write|action)\b/i },
  { boundary: "R5_EXTERNAL_COST", ruleId: "R5-EXTERNAL-COST", pattern: /\b(paid|billable|billing|cost|charge|expensive)\b[\s\S]{0,100}\b(api|resource|request|call|execution)\b|\b(api|resource|request|call|execution)\b[\s\S]{0,100}\b(paid|billable|billing|cost|charge|expensive)\b|\b(mass|bulk|large[- ]scale)\b[\s\S]{0,80}\b(api calls?|requests?|provision)\b/i }
];
