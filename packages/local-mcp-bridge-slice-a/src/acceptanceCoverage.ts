...
const evidenceByAc: Readonly<Record<number, readonly string[]>> = {
  ...
  24: ['POLICY:DataZoneDecisionRegistry', 'POLICY:DataZoneTargetBinding', 'FIXTURE:data-zone target scope mismatch'],
  ...
  27: ['POLICY:DataZoneDecisionRegistry', 'FIXTURE:production zone read'],
  ...
  32: ['POLICY:SearchBoundsFinitePositiveInteger', 'POLICY:SearchBoundsPolicyCeiling', 'FIXTURE:missing search bounds', 'FIXTURE:invalid fractional search bound', 'FIXTURE:search bound exceeds policy ceiling'],
  ...
  37: ['POLICY:DataZoneDecisionRegistry', 'POLICY:DataZoneAccessMapping', 'FIXTURE:untrusted development claim cannot authorize read'],
  38: ['POLICY:DataZoneDecisionRegistry', 'POLICY:TrustedConfigurationRegistry', 'FIXTURE:externally authorized trusted config can allow development read'],
  39: ['POLICY:DataZoneDecisionRegistry', 'FIXTURE:untrusted development claim cannot authorize read'],
  40: ['POLICY:DataZoneDecisionRegistry', 'FIXTURE:untrusted metadata may only tighten access'],
  47: ['POLICY:TrustedConfigurationRegistry', 'POLICY:TrustedConfigurationAuthority', 'FIXTURE:trusted config cannot self-authorize'],
  48: ['POLICY:TrustedConfigurationRegistry', 'POLICY:DataZoneAuthoritySemanticBinding', 'FIXTURE:externally authorized trusted config can allow development read'],
  49: ['POLICY:TrustedConfigurationRegistry', 'POLICY:TrustedConfigurationImmutableRef', 'FIXTURE:trusted config mutable exact ref rejected', 'FIXTURE:trusted config integrity failure'],
  ...
  55: ['POLICY:TrustedConfigurationRegistry', 'FIXTURE:trusted config cannot self-authorize'],
};
...
function makeEntry(number: number): AcceptanceCoverageEntry {
  ...
  if (!evidenceIds?.length) {
    return {
      ...
      evidenceType: 'MIXED',
      evidenceIds: [],
      rationale: `${acId} has no executable Slice A behavior beyond the locked definition boundary; it is not claimed as implemented evidence.`,
    };
  }
